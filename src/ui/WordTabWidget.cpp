...
WordTabWidget::WordTabWidget(ExtensionManager* extManager, RibbonEditorWidget* ribbonEditor, QWidget* parent)
    : QWidget(parent), m_extManager(extManager), m_ribbonEditor(ribbonEditor) {
    setupUi();
    refreshList();
}

void WordTabWidget::setupUi() {
    ...
    m_btnInstall = new QPushButton("➕ ដំឡើង Add-in ថ្មី (Install .dotm)", this);
    m_btnInstall->setObjectName("btnSuccess");
    connect(m_btnInstall, &QPushButton::clicked, this, &WordTabWidget::onInstallClicked);

    m_btnToggle = new QPushButton("⚡ បើក/បិទ (Toggle)", this);
    m_btnToggle->setEnabled(false);
    connect(m_btnToggle, &QPushButton::clicked, this, &WordTabWidget::onToggleStateClicked);

    m_btnEditRibbon = new QPushButton("🎨 កែសម្រួល Ribbon XML", this);
    m_btnEditRibbon->setEnabled(false);
    connect(m_btnEditRibbon, &QPushButton::clicked, this, &WordTabWidget::onEditRibbonClicked);

    m_btnBackup = new QPushButton("📦 Backup Add-in", this);
    m_btnBackup->setObjectName("btnSecondary");
    m_btnBackup->setEnabled(false);
    connect(m_btnBackup, &QPushButton::clicked, this, &WordTabWidget::onBackupClicked);

    m_btnUninstall = new QPushButton("🗑️ លុប (Delete)", this);
    m_btnUninstall->setObjectName("btnDanger");
    m_btnUninstall->setEnabled(false);
    connect(m_btnUninstall, &QPushButton::clicked, this, &WordTabWidget::onUninstallClicked);

    auto* btnRefresh = new QPushButton("🔄 Refresh", this);
    btnRefresh->setObjectName("btnSecondary");
    connect(btnRefresh, &QPushButton::clicked, this, &WordTabWidget::refreshList);

    m_btnOpenWord = new QPushButton("🚀 បើក Microsoft Word", this);
    m_btnOpenWord->setObjectName("btnSecondary");
    connect(m_btnOpenWord, &QPushButton::clicked, this, &WordTabWidget::onOpenWordClicked);
    ...
    toolbar->addStretch();
    toolbar->addWidget(m_btnOpenWord);
    ...
    connect(m_table, &QTableWidget::itemSelectionChanged, this, &WordTabWidget::onSelectionChanged);
...
void WordTabWidget::refreshList() {
    if (!m_extManager) return;

    m_items = m_extManager->scanWordExtensions();
    ...
    for (int i = 0; i < m_items.size(); ++i) {
        ...
        if (item.hasCustomRibbonXml) {
            ribbonItem->setForeground(QColor("#0078d4"));
        }
    ...
    m_lblStatus->setText(QString("បានរកឃើញ Add-in ចំនួន %1 ក្នុង Word Startup Folder").arg(m_items.size()));
    onSelectionChanged();
}

void WordTabWidget::onSelectionChanged() {
...
void WordTabWidget::onToggleStateClicked() {
    int row = m_table->currentRow();
...
void WordTabWidget::onEditRibbonClicked() {
    int row = m_table->currentRow();
    if (row < 0 || row >= m_items.size()) return;
...
void WordTabWidget::onInstallClicked() {
    QString filePath = QFileDialog::getOpenFileName(this, "ជ្រើសរើស Word Add-in Template", "", "Word Templates (*.dotm *.dotx *.wll);;All Files (*)");
    if (filePath.isEmpty()) return;

    QString error;
    if (!m_extManager->installExtension(filePath, "Word", error)) {
        QMessageBox::critical(this, "កំហុសដំឡើង", error);
    } else {
        QMessageBox::information(this, "ជោគជ័យ", "បានដំឡើង Add-in ចូល Word Startup រួចរាល់!");
        refreshList();
    }
}

void WordTabWidget::onUninstallClicked() {
    int row = m_table->currentRow();
    if (row < 0 || row >= m_items.size()) return;
...
void WordTabWidget::onBackupClicked() {
    int row = m_table->currentRow();
    if (row < 0 || row >= m_items.size()) return;
...
void WordTabWidget::onOpenWordClicked() {
#ifdef Q_OS_MACOS
